...
```{r, fig.cap = "Trace plots for all $\\lambda$ values and filtered for K = 2. Overall the trace plots show that the samplers are mixing well with some minor autocorrelation that could potentially be resolved with thinning or more samples. The raw trace plots include the $\ambda$ values when there are 3 and 4 separate $\\lambda$'s and is the cause of the random jumps.", fig.asp=0.8}
burnin = 400000
...
```{r, fig.asp = 1, fig.cap = "\\label{fig:2c_dens} Density plots for the count data, $K$", include = FALSE }
...
hist(res_save$K[res_save$K < 12],breaks = seq(-1,12,1), freq = FALSE)
