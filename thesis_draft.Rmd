...
$$Y_{i,t}|G \sim ~ Pois\big(\lambda_t*\sum_{j\neq i}^{N_v}Z_{j,t}*1[e_{ij}=1]+ \lambda_tZ_{i,t}\big)|G $$
...
Similar reductions in comptutation could feasibly be done following changes in the graph. Following a graph proposal any vertices whose component does not change, does not need its contribution to the likelihood recomputed. With sparse graphs this could save computational effort.

If we maintain a list of vertices (label them numerically) associated with the component they belong to (alphabetically/hex). We additionally maintain a list of components associated with a vector of vertices that comprise the component. Now assume we add an edge between two vertices. Then say a graph proposal is made to connect two vertices. We can look up the component that each of these vertices participate in (call them A and B) , merge the components (A, B -> C). We then look up the vertices that participate in components formerly known as A, B and update them to the new component name (as well as update the component list) maintaining the data structure. Then we know which vertices to update.

Deleting an edge is problematic. The edge is within a component and that component needs to be updated. Updating the components becomes the issue. Either the component remains connected or it's split into two components. There's no way (?) to determine which is the case without ()
Wikipedia 
"There are also efficient algorithms to dynamically track the components of a graph as vertices and edges are added, as a straightforward application of disjoint-set data structures. These algorithms require amortized O(alpha(n)) time per operation, where adding vertices and edges and determining the component in which a vertex falls are both operations, and alpha(n) is a very slow-growing inverse of the very quickly growing Ackermann function. A related problem is tracking components as all edges are deleted from a graph, one by one; an algorithm exists to solve this with constant time per query, and O(|V||E|) time to maintain the data structure; this is an amortized cost of O(|V|) per edge deletion. (Shiloach & Even 1981)."
...
The `birth_theta()` and `death_theta()` operators allow for increasing and decreasing the number of changepoints. Since the parameter space can jump between a collection of possible models $\{M_K, K \in \{0,1,\cdots,n-1\}\}$ where $K$ indexes the number of changepoints. However models from different $M_K$'s have different length of $\boldsymbol{\theta_k}$ and the likelihoods are not directly comparable, they need to be defined on the same probability space. To handle this we use a Reversible Jump MCMC (RJMCMC) as proposed in @green. 
...
where $\theta_m$ is the theta value that was removed.
...
The actual algorithm works as sfollows. The current number of edges $N_e$ is tracked. Then we randomly sample from $A =\{1,\cdots, N_v(N_v-1)-2*N_e\}$ then tracking the length of the adjacency lists, we decrement A by the $N_v - length(adj[i])$ where length(adj[i]) is the length of the adjacency list of vertex 1 and $i\in\{1,\cdots,N_v\}$. If $A >= 0$ then $i = i+1$ while while continuing to decrement A until $A < 0$. Then $i$ is the index of the adj which has the edge that needs to be added. Then perform a linear search until the remaining i'th index is removed.
...
2. for i in (1:N_v):
3. if (A < length(adj[i]))
4. perform linear search to find the first A - length(adj[i]) edge not in the list
5. else A = A-length(adj[i])

Then the probability of selecting an edge is $2/(N_v(N_v-1) - 2*N_e)$ by symmetry, each edge is represented twice.

$$ \frac{\frac{1}{N_e + 1}}{\frac{1}{M - E}} = \frac{M-E}{N_e + 1} $$
and the Hastings ratio for removing an edge is given by

$$ \frac{\frac{1}{N - (E - 1)}}{\frac{1}{ E}} = \frac{edges}{N_E-(E-1)} $$
## Delete edge

The `delete_edge_op` samples uniformly at random an edge currently in the graph and proposes to remove it.
