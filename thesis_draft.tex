...
\usepackage{lmodern}
\usepackage{amssymb,amsmath, natbib, soul}
...
In this - \dw{thesis} we \dw{perform inference on an extension of a model} presented by \citet{held2006} for modeling
parameters of infectious disease counts in a Bayesian framework. Their
method models the count data \dw{counts of what?} as a branching Poisson process with a
cyclical endemic parameter.

We extend their model to incorporate a graph \dw{what does graph represent}.
...
\dw{describe a basic SIS or SIR epidemic model here. A decent reference for all this stuff is \citet{keeling2011} - the library has a copy. Or look at eg refs used inthe Held paper or in the epinet papers.}
