...
\dw{you need to sort out the referencing and get some proper references in here, v important for this sort of work. Looks easier to do this direct in latex rather than via rmd?}
...
for modeling \st{parameters} of infectious disease counts in a Bayesian
framework. The \dw{origianl} method models surveillance data of infectious disease
...
(e.g.~seasonal flu patterns) and occasional ``outbreaks'' \dw{outbreaks are not modeled using the cyclical endemic parameter} (e.g.~swine
flu, H1N1). Their method models a single time series of count data from
a specific location or region. We attempt to extend their model to
multiple time series of count data that are spatially related.
...
developed severe complications and over 50 people died.
...
mechanistic models (models that use expert knowledge of the underyling
process) such as the Susceptible-(Exposed)-Infectious-Recovery
(SIR/SEIR) model and other variants. In the SEIR model, the entire
closed population of individuals is in one of the four states.
susceptible individuals can become exposed when in contact with an
infectious invidual, exposed individuals will eventually become
infectious and infectious individuals will eventually be recovered and
cannot be re-exposed or infect susceptibles (e.g.~by immunity). Given
partial data on the susceptible, exposed, infectious and removed times
parameters surrounding the epidemic of interest can be estimated. For
example in ({\textbf{???}}), the time spent in each state is modeled
with a Gamma random variables and the parameters of the random variables
can be estimated from the data. Other parameters of interest such as the
intial infected can also be estimated. They also estimate contact
network between individuals, covariates affecting the formation of the
contact network and a corresponding transmission tree. While effective,
this sort of mechanistic model requires data at a granularity not
typical of surveillance data, (such as information on the susceptibles)
and often contains other problems such as underreporting
({\textbf{???}})
...
({\textbf{???}}) of population over time over time. Suppose there's
currently one parent alive. Then in a branching process, this individual
would give birth to a random number of offspring and then immediately
die. Then at the next step, each of those offspring become parents who
...
For a Poisson process, when \(\lambda > 1\) an outbreak occurs. When
...
\section{\dw{Multiple locations connected by a graph}Graph Data Introduction}\label{graph-data-introduction}}
...
This dependency between cities is represented with a fixed graph \dw{change to nooation here - no point in introducing two subscripts never to use them. Just use $G = (V, E)$ as normal}
...
\(1[e_{ij}=1]\) if \(\{i,j\} \in E(G)\) and 0 otherwise. \st{For clarity,
the subscripts  will be dropped and it's assumed that the true
...
include the additional counts from connected cities as \dw{don't need * in these expressions}
...
\dw{downsample the plots here --- they include loads of points so are large, just plot eg every 10th or 100th point to make more manageable}
