...
for \del{modeling parameters of} infectious disease \dw{surveillance data}\del{counts in a Bayesian
framework}. Their method models \del{surveillance data of} infectious disease
\del{counts} \dw{incidence (i.e., new cases) --- actually check this, is it incidence or prevalance (the total number currently infected) being used, prevalance make more sense for an autoregressive model but not sure } as a branching Poisson process with a cyclical endemic parameter.
The cyclical endemic parameter models ``seasonality'' \del{of disease counts}
(e.g.~seasonal flu patterns) \del{and}\dw{while} the Poisson branching process \dw{allows for} 
\del{occasional} ``outbreaks'' (e.g.~swine flu, H1N1). Their method models a
single time series of count data from a specific location or region. We
\del{attempt to} extend the\del{ir} model to multiple time series of count data that
are spatially related.

\del{Specifically} The dataset of interest contains data from 412
...
\protect\hyperlink{ref-noauthor_ehec_2011}{2011} , pp.~2-3, 11). \dw{fix the citation here --- see suggested citation in the pdf}

We \del{attempt to} extend the\del{ir} model to incorporate a graph \del{. The graph}\dw{that}
...
\dw{def needs expanding} Statistical methods to model infectious disease data typically use
...
disease counts that serves as the basis of the the extended graph model.
...
two-component model allows for \del{the} \(\lambda\) to change over time
\del{allowing for the disease to change}\dw{representing changing} infectivity\del{ over time}.
...
243-255) of population over time\del{ over time}. Suppose there's currently
one parent alive \dw{make conncetion between alive/parent/child language here and analogous epidemiological language}. Then in a branching process, this individual would
...
Let \(Z = (Z_0, Z_1, ..., Z_n)\) be the \del{infectious disease counts}\dw{incidence} at
each time step \(t\) \dw{and let} each \del{\(Z_t\) is determined as} \(Z_t = Y_t + X_t,\ t \in \{1,\dots, N\}\) \del{W} where \(Y_t\) is the epidemic component and \(X_t\) is the endemic
component. The model is then specified through
\(Z_t | Z_{t-1}\).
...
\dw{use punctuation as normal in displayed equations, so above equation should have a comma after it and the following "where" does not have a capital --- this applies throughout} Where \(\lambda_t\) is the time varing infectivity parameter and
...
In this model,\del{ the} \(\lambda_t\) is allowed to vary over time. \dw{We restrict $\lambda$ to be piecewise constant with $K$ changepoints at locations }\del{The
...
\subsection{TK add references below \dw{what is this heading?}}\label{tk-add-references-below}}

\dw{If we consider only the process $Z_t = Y_t$}\del{For a Poisson process}, when \(\lambda > 1\) an outbreak occurs (Held et
...
and quar\dw{a}ntine procedures can cause the overall infectivity to decrease
below 1.
...
T\del{hen t}he probability of the full time series \(Z\) given the initial
starting count \(Z_0\) can be factored as a product of the probabilities
of each \(Z_t\) given the previous count \(Z_{t-1}\)\dw{,}
...
\(Z_t\) to a multiple time series of counts \(Z_{i,t}\) where \(i\) \del{now}
indexes separate time series. In this case we will say the \(i\) indexes
individual ``cities'' \del{where}\dw{so} \(Z_i\) represents the infectious disease
counts in city \(i\). Now, a city \(i\)'s epidemic disease count at time
\(t\) is modelled as a function of both it\del{'}s counts \(Z_{i,t-1}\) and
possibly other cities\dw{'} counts as well.

This dependenc\dw{e} between cities is represented \del{with}\dw{by} a \del{fixed} graph \(G\)
where \(N_v\), the number of vertices in graph is \del{fixed and }equal to the
number of cities and \dw{and the (undirected) edge $\{i,j\}$ connects vertices $i$ and $j$ when transmission of the disease in city $i$ influences that in city $j$ and vice versa --- (or write something to that effect... and maybe move it to end of this paragraph)}.\del{ \(N_e\) is the number of edges in the graph,
...
function of \(Z_{i,t-1}\) (as before, it's \dw{its} own counts at time \(t-1\))
...
include the additional counts from connected cities as \dw{don't need * in below}
...
That is\dw{,} the epidemic component for city \(i\) is the sum of all counts
...
more realistic, a migration parameter \(m \in (0,1)\) is introduced \dw{so that}
\del{The migration parameter enters into the likelihood as}
...
\dw{move this to a discussion at the end, just a distraction here}
...
\subsection{Graph Likelihood \dw{it is really a graph prior and the prior you discuss below is a prior on $p$, should move down or call this Graph model}}\label{graph-likelihood}}
...
In Bayesian analysis, we aim to estimate the \dw{posterior} distribution of the
parameters \del{on interest given the data}\dw{which we can calculate} via Bayes' theorem:
\[ P(\theta|D) = \frac{P(D|\theta)P(\theta)}{P(D)}. \] \(P(\theta|D)\) is
the posterior distribution of the parameters, \(\theta\)\dw{,} given the data
...
certain values. \dw{If we have little prior information about a parameter, we try to choose an uninformative prior to reflect that ignorance}\del{The prior distribution can be specified to by
...
\(\int P(\text{data}|\text{parameters})P(\text{parameters})\) \dw{why use words here rather than $D, \theta$ etc?} over the
...
methods are used \dw{which we discuss in Section ...}. 

\dw{maybe at this state the basci form of the posterior here --- i.e. replace generic $\theta$ and $D$ used above with the papameters and data for this model as well as factorising it then get on to specifying priors in section below} We now specify the priors of the Two-Component and
graph portions of the model.

\hypertarget{two-component-priors }{%
\subsection{Two-Component Priors \dw{Priors for $\lambda, \gamma,...$ etc}}\label{two-component-priors}}



The prior values for the \(\gamma\) parameters are Multivariate-Normal
distributed with variance \(3I_3\) to describe the uncertainty. \dw{do you mean you are modeling the $\gamma_i$ and independent each with a standard normal prior?}

\[\gamma_i \sim N(0, 3I_3), i \in \{0,1,2\}\]

Since the \(\lambda\) values parameterize a Poisson distribution we set
the prior to be Gamma(1, 1), it's \dw{its not it's --- check this throughout!} conjugate distribution. If Gamma is
interpreted as the sum of exponentials, then the shape = 1, rate = 1
parameterization represents seeing a single occurrence in 1 unit of time
and represents a vague prior.

\[ \lambda^{(k)} \sim Gamma(1, 1),\ k \in \{1, \dots, K + 1\} \]

\dw{I thought we were just sticking with a fixed number of change points?} The number of change-points \(K\) takes values in \(\{1,\dots,N\}\) where
\(N\) is the total number of time points of counts collected. \dw{edits got to about here...} In the
Held et al. (\protect\hyperlink{ref-held_two-component_2006}{2006})
paper the number of changepoint \dw{spelling: two words or hyphenated} is uniformly distributed
...
To \dw{approximate the posterior, we draw samples from it using}\del{compute samples from the posterior distribution, } the
Metropolis-Hastings algorithm (and its variant, the
Metropolis-Hastings-Green algorithm)\del{ is used}. The algorithm begins with
