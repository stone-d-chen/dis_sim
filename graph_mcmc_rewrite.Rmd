...
Need to pass both adjacency lists and then recompute for each of those halves

```{r}
compute_log_like_rat <- function(input, old_adj, v1, v2) {
  K = input$K
  theta = input$theta
  lambda = input$lambda
  gamma = input$gamma
...
#-------for prior testing--------
 ...
 # compute_log_like_rat <- function(input, adj, v1,v2){
 #   0
 # }
```
...
del_edge_op <- function(input){
  ...
  hastings <- log(2) + log(num_edges) - log(ord*(ord - 1) - 2*(num_edges - 1))
...
output_LL <- compute_log_like(output)
...
burnin = 50000
downsample = seq(1, nSim-burnin, length=10000)
edge_hist = list()
edge_hist$count <- res$count[-(1:burnin)]

#save(edge_hist, file = "prior_graph.RData")

par(mfrow = c(1,2))
plot(graph.adjlist(output$adj, mode = "all"))
mean(res$accept[-(1:burnin)] == 1)
hist(res$count[-(1:burnin)][downsample], breaks = 100)
