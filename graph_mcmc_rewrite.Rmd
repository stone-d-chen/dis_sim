...
compute_log_prior <- function(input){
  ...
  count = input$ec[["count"]]
   ...
   prior_star <- -log( choose( choose(length(adj), 2), count))
  # -log(choose(N, K)) +
...
change_migration <- function(input) {
  ...
  migration_star <- migration + runif(1, -0.2, 0.2)
...
add_edge_op <- function(input){
  ...
  v1 <- sample(1:ord, 1, prob = probs)

  
  v2 <- sample(setdiff(1:ord, adj[[v1]]))
  ...
  input_star$ec <- input$ec
  input_star$ec$count <- input$ec$count + 1
  
  
  
  LL_star <- compute_log_like(input_star)
  prior_star <- compute_log_prior(input_star)
  hastings <- log( choose(ord, 2) - num_edges )  -log(num_edges + 1)
  post_star <- LL_star + prior_star #new log posterior
...
del_edge_op <- function(input){
  ...
  LL_star <- compute_log_like(input_star)
  prior_star <- compute_log_prior(input_star)
  hastings <- -log(choose(ord, 2) - (num_edges - 1) ) + log(num_edges)
  post_star <- LL_star + prior_star #new log posterior
  alpha <- post_star - LL + hastings
...
update_add_del_rewire <- function(input) {
...
ops = c(death_theta,
        ...
        compute_log_like)
lapply(ops, undebug)
#debug(change_migration)
# debug(compute_log_like)
...
system.time(for (i in 1:nSim) {
  if (i < 5) {
    ...
    output <-  update_del_add(output)
  } else {
    output <- update_add_del_rewire(output)
  }
