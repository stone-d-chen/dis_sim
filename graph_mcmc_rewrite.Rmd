...
cppFunction("double mypoisC(NumericVector data, NumericVector lambdas) {
  int n = data.size();
...
cppFunction("NumericVector diff_sug(NumericVector x){
   return diff(x);
}")
...
# ER random graph
g_15 = sample_gnp(15, 0.11)
plot(g_15)

adjlist = as_adj_list(g_15, mode = 'all')
...
counts_city <- simulate_graph_data(init, N, K, theta, migration, lambda, INDEP, gamma, g_15)



simulate_graph_data <- function(init, N, K, theta, migration, lambda, INDEP, gamma, graph, adj = NULL) {
...
change_migration <- function(input) {
  ...
  migration_star <- migration + rnorm(1, 0, 0.01)
...
system.time(for (i in 1:nSim) {
  ...
  output <-  update_param_mig(output)
  } else {
...
plot(res$accept[900000:1000000], type = "l")
