...
actual <- list(K = K, theta = theta, lambda = lambda, LL = -Inf,
...
flip_edge <- function(input) {
  ...
  if (ep[1] %in% adj[[ ep[2] ]]) {
    m = which(adj[[ ep[2] ]] == ep[1])
    adj[[ ep[2] ]] = adj[[ ep[2] ]][-m]
    
    m = which(adj[[ ep[1] ]] == ep[2])
    adj[[ ep[1] ]] = adj[[ ep[1] ]][-m]
    
    #ec = update_edge_c(ec, ep[1], ep[2], "del")

  } else {
    adj[[ ep[2] ]] = sort(c(adj[[ep[2]]], ep[1]))
    adj[[ ep[1] ]] = sort( c(adj[[ ep[1] ]], ep[2]))
    
    #ec = update_edge_c(ec, ep[1], ep[2], "add")
  }
  input_star <- input
  input_star$adj <- adj
  input_star$ec <- ec
...
```{r}
ops = c(death_theta, birth_theta, change_theta, change_lambda, change_gamma, flip_edge, compute_like)
lapply(ops, undebug)
...
output <- list(K = 3, theta = c(10, 25, 150), lambda = c(0.5, 1, 0.5,  0.5), LL = -Inf,
               gamma = gamma, INDEP = INDEP, adj = adjlist, graph = 1)
...
update_edge_c <- function(elist, v1, v2, update_t){
  ...
  increment_edge_c <- function(elist, v1, v2) {
  ...
  if (update_t == "del") {
    elist = decrement_edge_c(elist, v1, v2)
  } else if (update_t == "add") {
    elist = increment_edge_c(elist, v1, v2)
  }
