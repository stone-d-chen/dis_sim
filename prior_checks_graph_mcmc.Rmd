...
compute_prior <- function(input){
    ...
    adj = input$adj
    0
    prior_star <- -log(choose(N, K)) + sum(dgamma(lambda, 1, 1, log = TRUE)) + sum(dnorm(gamma, 0, 3, log = TRUE))
}
```
...
death_theta <- function(input) {
    ...
    alpha <- post_star  - LL  + proposal + log_jacobian
...
change_lambda <- function(input) {
  ...
  lambda_star <- lambda
  lambda_star <- lambda_star + rnorm(K + 1, 0, 0.25)
...
```{r}
ops = c(death_theta, birth_theta, change_theta, change_lambda, change_gamma, compute_prior)
lapply(ops, undebug)
...
inittheta = seq(1,N, by = 2)

output <- list(K = length(inittheta), theta = inittheta, lambda = rep(0.5, length(inittheta) + 1), LL = -Inf,
               gamma = gamma, INDEP = INDEP, graph = g_city, p = 0.5, adj = adjlist)
...
hist(res$K[-(1:burnin)],freq = FALSE)
...
plot(density(res$gamma[-(1:burnin), 1], bw=1))
plot(density(res$gamma[-(1:burnin), 2], bw=1))
plot(density(res$gamma[-(1:burnin), 3], bw=1))
...
hist(res$K)
plot(density(res$lambda[-(1:burnin), 1], bw=0.01))
lines(dgamma(seq(0,14,length = 1000),1,1),seq(0,14,length = 1000), col = "red")
plot(density(res$lambda[-(1:burnin), 2], bw=0.01))
plot(density(res$lambda[-(1:burnin), 3], bw=0.01))
plot(density(res$lambda[-(1:burnin), 4], bw=0.01))
title("Actual, #CP and Lambda densities", outer = TRUE)
